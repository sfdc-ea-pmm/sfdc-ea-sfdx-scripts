...
## Include Sample Data in Scratch Org?
...
print_usage() {
  echo
    echo "Usage:"
    echo "  initOrg.sh -d <DURATION> -s <INCLUDE SAMPLE DATA> -t <TEMPLATE API NAME>"
    echo
    echo "Arguments:"
    echo "    -d    [optional] set scratch org duration [default: $ORG_DURATION]"
    echo "    -s    [optional] include sample data in scratch org [default: $INCLUDE_SAMPLE_DATA]"
    echo "    -t    [optional] set if working on a existing template. template api name to deploy, should be same as folder name under waveTemplates/. "
    echo
}

while getopts 'd::st::' flag; do
  case "${flag}" in
    d) ORG_DURATION="${OPTARG}" ;;
    s) INCLUDE_SAMPLE_DATA=true;;
...
# print args
echo "ORG_DURATION=${WARN}$ORG_DURATION${NC} INCLUDE_SAMPLE_DATA=${WARN}$INCLUDE_SAMPLE_DATA${NC} TEMPLATE_API_NAME=${WARN}$TEMPLATE_API_NAME${NC}"
...
if [[ ! -z $TEMPLATE_API_NAME ]]
then
    # get template ID
    TEMPLATE_ID="$(sfdx analytics:template:list | grep "$TEMPLATE_API_NAME" | sed 's/  /,/g' | cut -d ',' -f3)"
